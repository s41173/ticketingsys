...
class Ledger extends MX_Controller
{
    ...
    private $user, $period, $member;

    function index(){ $this->session->unset_userdata('start'); $this->session->unset_userdata('end'); $this->start(); }
	...
	// ---------------------------------------- //
        ...
        if($acc){ $ledgers = $this->Model->get_monthly($this->account->get_id_code($this->session->userdata('member'),$acc),$ps->month,$ps->year)->result(); }
        else { $ledgers = null; }
        ...
        // balance
        if ($acc){ $bl = $this->get_balance($this->account->get_id_code($this->session->userdata('member'),$acc));
          $data['begin'] = $bl[0];
          $data['end'] = $bl[1];
          $data['mutation'] = $bl[2];
          $data['debit'] = $bl[3];
          ...
          // ===== chart  =======
          $data['graph'] = site_url('ledger')."/chart/IDR/".$this->account->get_id_code($this->session->userdata('member'),$acc);
          $data['source'] = site_url($this->title.'/getdatatable');
          
          $data['accname'] = $acc.' : '.$this->account->get_name($this->account->get_id_code($this->session->userdata('member'),$acc));
          
        }else{
          $data['graph'] = null;
          $data['source'] = null;
          $data['begin'] = '0'; $data['end'] = '0'; $data['mutation'] = '0'; $data['debit'] = '0'; $data['credit'] = '0';  
        }
	...
	// ---------------------------------------- //
        ...
        $accname = null; if($acc){ $accname = $this->account->get_name($this->account->get_id_code($this->session->userdata('member'),$acc)); }

        if($acc){ $ledgers = $this->Model->get_ledger($this->account->get_id_code($this->session->userdata('member'),$acc),$start,$end)->result(); }
        else { $ledgers = null; }
...
//        $data['graph'] = $this->chart($this->input->post('ccurrency'),$this->account->get_id_code($acc));
        ...
        // balance
        $bl = $this->get_balance($this->account->get_id_code($this->session->userdata('member'),$acc));
        $data['begin'] = $bl[0];
        $data['end'] = $bl[1];
        $data['mutation'] = $bl[2];
        $data['debit'] = $bl[3];
...
//        
    ...
    public function chart($cur='IDR',$acc=null)
    {
        ...
        $gl = new Gl();
        $bl = new Balances();
        $ps->get($this->session->userdata('member'));
        
        if ($acc){
            $gl = $this->Model->get_ledger($acc,$this->session->userdata('start'),$this->session->userdata('end'))->result();
...
// ====================================== REPORT =========================================
    ...
    //        Property Details
        ...
        $data['accounts'] = $this->Model->report($this->account->get_id_code($this->session->userdata('member'),$accstart),$this->account->get_id_code($this->session->userdata('member'),$accend))->result();
